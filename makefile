...
# 
# make coff = 转换 ELF 调试文件到 AVR 的 COFF 调试文件.适用于AVRStudio.3.X及以前的版本. VMLAB <3.10 
# 
# make extcoff = 转换 ELF 调试文件到 AVR 的 Extended COFF 调试文件.  
# VMLAT 3.10+ 适用于AVRStudio.4.07 及以后的版本。也就是现在我们要使用的版本
...
F_CPU = 7372800
# 输出文件格式，对于单片机编程来说，一般我们选intel 的hex格式. (可以是: srec, ihex, binary)
FORMAT = ihex
...
SRC = delay.c eerom.c  setup.c  kkavr.c  gyro.c  motor.c ppm.c
# 列出你的 C++工程中的所有的 *.cpp源文件名. (C++的所有的文件依赖关系也会自动建立.)
...
CDEFS = -DF_CPU=$(F_CPU)UL -DATMega168
# 这个和上面的说明一样，不过是对于C++文件的
CPPDEFS = -DF_CPU=$(F_CPU)UL -DATMega168
#----------------------- C 编译选项-------------------------------
#  -g*:          产生调试信息
#  -O*:          代码优代级别
...
#----------------------外部存储器选项---------------------------
#      关于--defsym 的说明
#      --defsym symbol=expr   #定义一个全局的标志,用expr作为symbol的值.
#    -Tbss org    
#    -Tdata org
#    -Ttext org         
...
AVRDUDE_WRITE_FLASH = -p m168p -c usbasp -U flash:w:$(TARGET).hex
...
%.eep: %.elf
	@echo
	@echo $(MSG_EEPROM) $@ -$(OBJCOPY) -j .eeprom --set-section-flags=.eeprom="alloc,load" \
