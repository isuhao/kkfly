...
void SetupFilter(void) {
	...
	while (1) {
		...
		//根据摇杆的高低设置对应参数的开关
		if (RxRud > STICKGATE)
			BITSET(rev, SOFT_FLT);  //Rud is filter on/off
		if (RxRud < -STICKGATE)
			BITCLR(rev, SOFT_FLT);

		if (RxEle > STICKGATE)
			BITSET(rev, SOFT_EXP);  //Ele is exp on/off
		if (RxEle < -STICKGATE)
			BITCLR(rev, SOFT_EXP);
...
void Setup(void) {
	...
	while (1) {
		...
		if (RxThr < 30) {
			...
			case 1:
				SetupThrTravel();
				break;	//1.Motor thr cali
			case 2:
				SetupFilter();
				break;	//2.Software filter on/off
			case 3:
				SetupGyroDir();
				break;	//3.Gyro direction reverse
			case 4:
				SetupAxisMode();
				break;	//4.Axis mode
			case 5:
				SetupResetAll();
				break;	//5.Reset to empty
			}
		}
	}
}
