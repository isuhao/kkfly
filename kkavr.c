...
void ArmingRoutine(void) {
	...
	if (RxRud < -STICKGATE || RxRud > STICKGATE)
		ArmCnt++;
	else
		ArmCnt = 0;
...
void CaclAttitude(void) {
	...
	if (GyroBaseCnt) {
		...
		GyroBaseCnt--;
		if (!(GyroBaseCnt & 7))
			LED0_TOG();	//Shine LED show gyro cali 闪烁LED表示在进行陀螺仪校准
		...
		if (InLock) {
			...
			//根据设置反转各个陀螺仪信号
			if (BITTST(DevRev,GYRO_ROL))
				GyroRol = -GyroRol;
			if (BITTST(DevRev,GYRO_PIT))
				GyroPit = -GyroPit;
			if (BITTST(DevRev,GYRO_YAW))
				GyroYaw = -GyroYaw;
